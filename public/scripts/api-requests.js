...
const makeOMDBRequest = (query) =>
  $.get(`/api/omdb/${query}`).fail((err) => console.log(err.message));
...
const makeTMDBRequest = (query) =>
  $.get(`/api/tmdb/${query}`).fail((err) => console.log(err.message));
...
const makeGBooksRequest = (query) =>
  $.get(`/api/gbooks/${query}`).fail((err) => console.log(err.message));
 ...
 */
const makeIPRequest = () =>
  $.get(`/api/ipapi`).fail((err) => console.log(err.message));
...
const makeYelpRequest = (query) =>
  makeIPRequest()
    .then((location) => {
      ...
      return $.get(`/api/yelp/${query}/${latitude}/${longitude}`).fail((err) =>
...
const makeAMZNRequest = (query) =>
  $.get(`/api/amazonprice/${query}`).fail((err) => console.log(err.message));
